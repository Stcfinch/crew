...
```
frontmatter → H1 標題 → 摘要 → 鐵律 → 紀律護欄 → 設定（檔/目錄）
→ 前置條件 → 使用方式 → 流程 → {特殊模式段} → 何時不用 → Gotchas → 邊界情況
```

- 尾段固定「何時不用 → Gotchas → 邊界情況」三段收尾（負向指引集中檔尾，不打斷檔頭的鐵律/紀律護欄；現況僅 plan-browse Gotchas/邊界情況反序，須改正）。
...
---
name: {skill-name}                                      # [必要]
description: {一句功能摘要}。當使用者提到 /指令名、「{組合詞1}」、「{組合詞2}」…時觸發此 Skill。   # [必要，尾句固定，須含「當使用者提到」]
...
## 設定檔                                                # [條件必要：內容未被 prerequisites.md 涵蓋時；純本地 skill（如 crew-upgrade）可免]
...
| 設定檔 | 條件必要：內容未被 prerequisites.md 涵蓋時 | 說明設定檔查找順序與缺檔導向 |
...
- 尾句固定：`當使用者提到 /指令名、「{組合詞1}」、「{組合詞2}」…時觸發此 Skill。`（斜線指令置首＋1-2 個組合詞；29/29 已一致，照抄）。**「當使用者提到」為 CI `scripts/lint-skill-contract.py` 硬性契約字串，不可改寫成「當使用者輸入…」等變體。**
