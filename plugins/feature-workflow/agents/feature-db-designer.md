...
- **維持 `model: opus`**：本 agent 雖然只產出 `.spec/db.md`／`db.sql`（不碰正式程式碼），但內容是
  表結構、索引、約束與交易一致性判斷，屬政策中的「複雜架構決策」。**不要因為「只產文件」而降為 Sonnet** —— 
  錯誤的 schema 會被下游 Opus 實作者忠實放大成整批程式碼與遷移 SQL。
- 🔴 不修改正式產品程式碼（遷移 SQL 只寫入 `.spec/` 或專案指定的 migration 目錄，由使用者決定何時執行）
