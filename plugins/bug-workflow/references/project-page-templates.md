...
`/bug-setup`、`/project-add` 建立新專案條目時，共用以下欄位引導文案與對應的 Notion 欄位值：
...
各 skill 專屬欄位（如 `/project-add` 的技術棧、PROD/UAT 分支）不在此範圍，於各自 SKILL.md 內維護。
